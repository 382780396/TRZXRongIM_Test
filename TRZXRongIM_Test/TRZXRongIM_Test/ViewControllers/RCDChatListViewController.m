...
//        [[KPOUserDefaults userId] length] < 1) {
//        return;
//    }
...
//        }
        ...
        if ([[RCDataBaseManager shareInstance] getAddFriendMessageCount]) {
            ...
            self.selectedAddressBook.badgeNumberLabel.text = [NSString stringWithFormat:@"%ld",[[RCDataBaseManager shareInstance] getAddFriendMessageCount]];
        }else{
...
-(void)willDisplayConversationTableCell:(RCConversationBaseCell *)cell atIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([cell isKindOfClass:[RCConversationCell class]]) {
        ...
        if (conversationCell.model.conversationType == ConversationType_APPSERVICE||
            ...
            view.image = [UIImage imageNamed:@"RCDConversation_Public_Message"];
...
- (IBAction)showMenu:(UIButton *)sender {
    ...
    @[
      [KxMenuItem menuItem:@"发起群聊"
                     image:[[UIImage imageNamed:@"RCD_LaunchGroup"] RC_scaleToSize:CGSizeMake(20, 20)]
                    target:self
                    action:@selector(pushChat:)],
      
      [KxMenuItem menuItem:@"添加好友"
                     image:[[UIImage imageNamed:@"RCDSearch"] RC_scaleToSize:CGSizeMake(20, 20)]
                    target:self
...
//    }
    ...
    selectPersonVC.clickDoneCompletion = ^(RCDSelectPersonViewController *selectPersonViewController,NSArray *selectedUsers){
        if(selectedUsers.count == 1){
            ...
            // 判断 - 不能与自己聊天
            // 身份判断
//            if ([user.userId isEqualToString:[KPOUserDefaults userId]]) {
...
//                [ProfileViewModel getAutInfoAuthUserId:[KPOUserDefaults userId] token:[KPOUserDefaults token] authUserId:@"" success:^(id object) {
...
//                        [LCProgressHUD hide];
        ...
        if([message.content isKindOfClass:[RCInformationNotificationMessage class]]){
            ...
            if ([notificationMessage.extra containsString:@"admin"] ||
                ...
                // 用户ID
//                if([notificationMessage.extra containsString:[KPOUserDefaults userId]]){
...
//                }
    ...
    if ([message.content isMemberOfClass:[RCContactNotificationMessage class]]) {
        ...
        [[RCDHttpTool shareInstance]getUserInfoWithUserId:message.senderUserId GetUserInfoSuccessBlock:^(id json) {
...
//        __weak __typeof(self) sfself = self;
        ...
        [_selectedFinancingLoop setButtonDidClickBlock:^(RCDSelectedViewType type) {
            ...
            // 跳转投融圈
//            TRZXFriendLineTableViewController *DynamicHomeVC  = [[TRZXFriendLineTableViewController alloc] init];
//            [sfself.navigationController pushViewController:DynamicHomeVC animated:YES];

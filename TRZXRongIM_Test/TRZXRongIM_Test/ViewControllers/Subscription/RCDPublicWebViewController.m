...
-(void)rightBarButtomItemPressed:(UIButton *)button{
    ...
    msg.image=self.tempImage.image?:[UIImage imageNamed:@"展位图"];//缩略图
...
//    msg.type=@"course"; // 分享类型
//    msg.objId= _mid;// 课程id
//    msg.headURL = _topDataModel.userPic;
//    
//    [[Kipo_ShareManager sharedManager]showTRZXShareViewMessage:msg];
}
...
-(UIImageView *)tempImage{
    if (!_tempImage) {
        _tempImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"展位图"]];

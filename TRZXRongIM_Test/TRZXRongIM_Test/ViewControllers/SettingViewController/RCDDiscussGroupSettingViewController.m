...
@interface RCDDiscussGroupSettingViewController ()<UIActionSheetDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
//            [self.groupHeadManager kipo_setGroupListHeader:self.tempGroupImage groupInfo:group isSave:YES];
            ...
            // 用户ID
            if ([@"" isEqualToString:group.adminId]) {// [KPOUserDefaults userId]
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    // 用户ID
    [[NSUserDefaults standardUserDefaults] setObject:data forKey:[NSString stringWithFormat:@"chatBackImage%@",@""]];//[KPOUserDefaults userId]
...
- (void)didTipHeaderClicked:(NSString*)userId{
    ...
    // 跳转个人主页
//    PersonalInformationVC *personalVC = [[PersonalInformationVC alloc]init];
//    personalVC.midStrr = userId;
//    personalVC.otherStr = @"1";
...
//    jubao.type =  ComplaintsType_Chat;
...
//    UIImage *newImage = [icImage kipo_imageCompressToTagetSize:CGSizeMake(RC_SCREEN_WIDTH, RC_SCREEN_HEIGHT)];
//    
//    NSData *data = UIImageJPEGRepresentation(newImage, 1);
//    
//    [[NSUserDefaults standardUserDefaults] setObject:data forKey:[NSString stringWithFormat:@"chatBackImage%@",[KPOUserDefaults userId]]];

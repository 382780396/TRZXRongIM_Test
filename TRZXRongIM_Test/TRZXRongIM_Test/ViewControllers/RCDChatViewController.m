...
@property(nonatomic,strong)BPHeadView *headView2;
...
//    [self registerClass:[RCUnknownMessageCell class] forCellWithReuseIdentifier:RCUnknownMessageTypeIdentifier];
    ...
    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_Picture"] title:@"照片" tag:PLUGIN_BOARD_ITEM_ALBUM_TAG];
    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_Camera"] title:@"拍摄" tag:PLUGIN_BOARD_ITEM_CAMERA_TAG];
    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_Voice"] title:@"语音通话" tag:PLUGIN_BOARD_ITEM_VOIP_TAG];
    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_Video"] title:@"视频通话" tag:PLUGIN_BOARD_ITEM_VIDEO_VOIP_TAG];
    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_BusinessCard"] title:@"分享名片" tag:PLUGIN_BOARD_ITEM_BUSINESSCARD_TAG];
    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_Location"] title:@"分享位置" tag:PLUGIN_BOARD_ITEM_LOCATION_TAG];
//    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_RedPacket"] title:@"红包" tag:PLUGIN_BOARD_ITEM_REDPACKETS_TAG];
    [self.chatSessionInputBarControl.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"RCDPluginBoard_Collection"] title:@"分享收藏" tag:PLUGIN_BOARD_ITEM_COLLECTION_TAG];
...
//            collectionVC.selectedPresent = YES;
//            collectionVC.conversationType = self.conversationType;
//            collectionVC.conversationTargetId = self.targetId;
...
//        LuYanInfo.projectId = mid;
//        [self.navigationController pushViewController:LuYanInfo animated:YES];
    }else if ([collectionType isEqualToString:@"investor"]||
              [collectionType isEqualToString:@"investorHome"]) {
        // 跳转投资人详情
//        Xiangqing2ViewController *xiangqingVC = [[Xiangqing2ViewController alloc]init];
//        xiangqingVC.XiangQingType = DetailedTypeInvestors2;
//        xiangqingVC.tzrid = mid;
//        [self.navigationController pushViewController:xiangqingVC animated:YES];
        ...
        // 跳转商业计划书
//        [KipoMyBusinessPlanViewModel getPlanApiDetailDataMID:mid success:^(id json) {
...
-(void)getTouchImage:(UIView *)view model:(RCMessageModel *)model{
    ...
    if ([view isKindOfClass:[UIImageView class]]) {
        ...
        if (model.messageDirection == MessageDirection_SEND) {
            ...
            UIImage * img= [UIImage imageNamed:@"chat_to_bg_highlighted"];
            ...
            UIImage *image = touthView.image;
            UIImage * img= [UIImage imageNamed:@"chat_from_bg_highlighted"];
            img = [img stretchableImageWithLeftCapWidth:image.leftCapWidth topCapHeight:image.topCapHeight];
            touthView.highlightedImage = img;
        }
...
- (void)didTapCellPortrait:(NSString *)userId{
    //跳转个人主页
//    PersonalInformationVC *privatePersonal = [[PersonalInformationVC alloc]init];
//    privatePersonal.midStrr = userId;
//    privatePersonal.otherStr = @"1";
//    [self.navigationController pushViewController:privatePersonal animated:YES];
}
...
//            
//            if ([headView.BPInfo.createById isEqualToString:[KPOUserDefaults userId]]) {
//                
//                switch (headView.BPInfo.status.intValue) {
//                    case 0:{//
//                        //发送网络请求
//                        [self.investorlistModel requestLoadData_updateByShare:self.headView.BPInfo.mid SuccessBlock:^(id json) {
...
//                case 0:{//
//                    //发送网络请求
//                    [self.investorlistModel requestLoadData_updateByShare:headView.BPInfo.mid SuccessBlock:^(id json) {
//                        [LCProgressHUD showSuccess:@"同意成功"];
//                        [sfself requestBP:nil];
//                    }];
//                }
...
//    if (self.isBPOpen) {
//        // 如果当前打开 则关闭
//        [KPOUserDefaults saveUserDefaults_Bool:NO objKey:@"BPViewShowKey"];
//        tiaoImage.image = [UIImage imageNamed:@"dowmTiao"];
...
//        
//    }else{
//        [KPOUserDefaults saveUserDefaults_Bool:YES objKey:@"BPViewShowKey"];
//        tiaoImage.image = [UIImage imageNamed:@"upTiao"];
//        // 如果当前关闭则 打开
//        [UIView animateWithDuration:0.2 animations:^{
//            self.BPBackgroundView.y += BPNumbers * 64;
//            sfself.conversationMessageCollectionView.y += 64 * BPNumbers;
...
//            //暂时没有
//            [self.investorlistModel requestLoadData_updateByShare:self.investorlistModel.investorsData.BPInfo.mid SuccessBlock:^(id json) {
//                [LCProgressHUD showSuccess:@"同意成功"];
//                [sfself requestBP:nil];
//            }];
//        }
...
//            [self.investorlistModel requestLoadData_saveByShare:self.targetId SuccessBlock:^(id json) {
...
//            if(self.conversationType == ConversationType_PRIVATE){
//                //查询
//                [self.investorlistModel requestLoadData_findInfo:self.targetId SuccessBlock:^(id json) {
//                    NSArray *temp = [InvestorsBPInfo mj_objectArrayWithKeyValuesArray:json[@"data"]];
//                    
//                    if (temp.count != 0) {
//                        BPNumbers = (int)temp.count;
//                        [self.view addSubview:self.BPBackgroundView];
//                    }
//                    
//                    // 进行判断
...
//                                self.headView.BPInfo = temp[i];
//                            }else{
//                                [self AddBPHeadViewBPHeadViewType:BPHeadViewTypeProject1 InvestorsBPInfo:temp[i]];
//                            }
//                        }
//                        
//                        if (i == 1) {
//                            if (self.headView2 != nil) {
//                                self.headView2.BPInfo = temp[i];
//                            }else{
//                                [self AddBPHeadViewBPHeadViewType:BPHeadViewTypeProject2 InvestorsBPInfo:temp[i]];
//                            }
//                        }
//                    }
//                    //滚到最下面一行
 ...
 *  添加BP headView
 */
//-(void)AddBPHeadViewBPHeadViewType:(BPHeadViewType)HeadViewType InvestorsBPInfo:(InvestorsBPInfo *)BPInfo{
//    
//    //    if (HeadViewType == BPHeadViewTypeProject1) {
//    if (!self.headView || !self.headView2) {
...
//        }
//        
//        if (HeadViewType == BPHeadViewTypeProject2) {
//            
//            _headView2 = headView;
//            _headView2.HeadViewType = BPHeadViewTypeProject2;
//            headView.frame = CGRectMake(0, 64, RC_SCREEN_WIDTH, 64);
//        }
...
//            LuYanProjectInfoViewController *projectVC = [[LuYanProjectInfoViewController alloc]init];
//            projectVC.projectId = headView.BPInfo.projectzId;
//            projectVC.LuYanInfo = DetailedTypeInvestorsSeeInfo;
//            [self.navigationController pushViewController:projectVC animated:YES];
//        }
//            break;
//        case HeadViewDirectionInvestor:{//投资人
//            Xiangqing2ViewController *xiangqing = [[Xiangqing2ViewController alloc]init];
//            xiangqing.XiangQingType = DetailedTypeInvestors;
//            xiangqing.tzrid = self.targetId;
...
//    vc.PDFUrl = url;
//    vc.titleText = title.length ? title : @"";
...
-(void)public_board_item_voip_tagDidClick{
    ...
    for (RCDUserInfo *userInfo in self.groupInfo.users) {
        // 用户ID
//        if(![userInfo.userId isEqualToString:[KPOUserDefaults userId]]){
...
-(void)replaceBakcground:(NSNotification *)notifi{
    ...
    // 聊天背景
    [[NSUserDefaults standardUserDefaults] setObject:data forKey:[NSString stringWithFormat:@"chatBackImage%@",@""]];//用户ID
}

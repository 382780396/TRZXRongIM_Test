...
@interface RongImUserLogin()<RCIMConnectionStatusDelegate,RCIMReceiveMessageDelegate,UIAlertViewDelegate,RCIMUserInfoDataSource,RCIMGroupInfoDataSource>

@end

@implementation RongImUserLogin

static RongImUserLogin *instance;
...
+(instancetype)shreadInstance{
    ...
    dispatch_once(&oneToken, ^{
        instance = [[RongImUserLogin alloc]init];
    });
...
- (void)getUserInfoWithUserId:(NSString *)userId completion:(void (^)(RCUserInfo *))completion{
    ...
    if ([userId containsString:@","]) {
        ...
        if (isPull) {
            ...
            for (NSString *newUserId in array) {
                [[RCDHttpTool shareInstance]getUserInfoWithUserId:newUserId GetUserInfoSuccessBlock:^(id json) {
        ...
        if (userInfo == nil) {
            ...
            [[RCDHttpTool shareInstance]getUserInfoWithUserId:userId GetUserInfoSuccessBlock:^(id json) {
...
//
//                RCDUserInfo *user = [[RCDataBaseManager shareInstance] getUserByUserId:[KPOUserDefaults userId]];
//                if (![user.name hasSuffix:@">"] && user.name.length) {
//                    [RCIM sharedRCIM].currentUserInfo = [[RCDataBaseManager shareInstance] getUserByUserId:[KPOUserDefaults userId]];
//                    [RCIMClient sharedRCIMClient].currentUserInfo = [[RCDataBaseManager shareInstance] getUserByUserId:[KPOUserDefaults userId]];
//                }else{
//                    [[RCDHttpTool shareInstance] updateUserInfo:[KPOUserDefaults userId] success:^(RCDUserInfo *user) {
 ...
 *  @param targetPicURL   目标对象头像 URL
 *  @param titleStr       目标对象 名字
 */
...
//    }
    // 用户ID
//    if ([targetId isEqualToString:[KPOUserDefaults userId]]) {
    // 融云提示
...
-(void)jumpRongCloudChatVCWithViewController:(UIViewController *)ViewController TargetId:(NSString *)targetId{
    ...
    if (RcUserInfo == nil) {
        ...
        // 查询目标 id 信息
        [[RCDHttpTool shareInstance] getUserInfoWithUserId:targetId GetUserInfoSuccessBlock:^(id json) {
...
-(void)RongCloudLoginWithToken{
    ...
    // 用户ID
//    NSString *keyStr = [KPOUserDefaults userId];
//    if (keyStr != nil) {

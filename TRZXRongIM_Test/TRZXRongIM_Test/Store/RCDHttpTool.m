...
-(void) getUserInfoByUserID:(NSString *) userID
        ...
        // 用户ID
//    1.获取 自己 的 信息
//    if ([userID isEqualToString:[KPOUserDefaults userId]]) {
//        
//        RCDUserInfo *userInfo = [[RCDUserInfo alloc]initWithUserId:userID name:[KPOUserDefaults name] portrait:[KPOUserDefaults head_img]];
//        if (completion) {
...
//    }else{
        ...
        if (RcUserInfo == nil || [RcUserInfo.name hasSuffix:@">"] || RcUserInfo.isAlso.length == 0) {
            ...
            [[RCDHttpTool shareInstance]getUserInfoWithUserId:userID GetUserInfoSuccessBlock:^(id response) {
 ...
 *  @param getUserInfoSuccessBlock 成功回调
 *  @param failureBlick            失败回调
 */
-(void)getUserInfoWithUserId:(NSString *)userId GetUserInfoSuccessBlock:(GetUserInfoSuccessBlock)getUserInfoSuccessBlock FailureBlick:(GetUserInfofailureBlick)failureBlick{
    ...
    [TRZXNetwork requestWithUrl:nil params:params method:POST   callbackBlock:^(id response, NSError *error) {
        if (response) {
            getUserInfoSuccessBlock(response);
        }
    }];
}

...
-(void)insertMsgFlag:(NSString *)msgFlagStr{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        ...
        // 用户ID
//        if ([db executeUpdate:updataSQL,@"1",[KPOUserDefaults userId]]) {
//
//        }
    }];
}
...
-(void)updateMsgFlag:(NSString *)msgFlagStr{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        ...
        // 用户ID
//        [db executeUpdate:insertSql,@"1",[KPOUserDefaults userId]];
    }];
}
...
//            break;
    ...
    [queue inDatabase:^(FMDatabase *db) {
        // 用户ID
//        [db executeUpdate:insertSql,@"0",[KPOUserDefaults userId]];
    }];
}
...
-(void)saveSpecialMsgFlag:(msgFlagType)msgFlagType{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        // 用户ID
//        [db executeUpdate:insertSql,@"2",[KPOUserDefaults userId]];
    }];
}
...
//            break;
    ...
    [queue inDatabase:^(FMDatabase *db) {
        // 用户ID
//        FMResultSet *rs = [db executeQuery:@"SELECT * FROM MSGTAHLEFLAG where msgTableId = ?",[KPOUserDefaults userId]];
//        while ([rs next]) {
...
//                [db executeUpdate:insertSql,@"0",[KPOUserDefaults userId]];
            ...
            [queue inDatabase:^(FMDatabase *db) {
                NSString *updataSQL = [NSString stringWithFormat:@"INSERT INTO MSGTAHLEFLAG (%@, msgTableId) VALUES (?,?)",msgFlagStr];
                // 用户ID
//                if ([db executeUpdate:updataSQL,@"0",[KPOUserDefaults userId]]) {
//                    
//                }
            }];
        }
...
-(void)insertVoiceRecordMessageId:(NSString *)messageId{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        ...
        // 用户ID
//        if ([db executeUpdate:updataSQL,messageId,[KPOUserDefaults userId]]) {
//            
//        }
    }];
}
...
-(BOOL)getVoiceRecordMessageId:(NSString *)messageId{
    ...
    [queue inDatabase:^(FMDatabase *db) {
         // 用户ID
//        FMResultSet *rs = [db executeQuery:@"SELECT * FROM VOICETABLE where userid = ?",[KPOUserDefaults userId]];
//        while ([rs next]) {

...
- (instancetype)initWithBrowseItemArray:(NSArray *)browseItemArray currentIndex:(NSInteger)currentIndex
{
    ...
    if(self)
    {
        ...
        _isEqualRatio = YES;
        _isFirstOpen = YES;
        _screenWidth = MSS_RC_SCREEN_WIDTH;
        _screenHeight = MSS_RC_SCREEN_HEIGHT;
...
- (void)initData
{
    for (MSSBrowseModel *browseItem in _browseItemArray)
    {
        ...
        if(_isEqualRatio)
        {
            ...
            {
                verticalRect = [browseItem.smallImageView.image mss_getBigImageRectSizeWithScreenWidth:MSS_RC_SCREEN_WIDTH screenHeight:MSS_RC_SCREEN_HEIGHT];
                horizontalRect = [browseItem.smallImageView.image mss_getBigImageRectSizeWithScreenWidth:MSS_RC_SCREEN_HEIGHT screenHeight:MSS_RC_SCREEN_WIDTH];
            }
        }
...
- (void)tap:(MSSBrowseCollectionViewCell *)browseCell
{
    ...
    if(browseItem.smallImageView)
    {
        ...
        if(_currentOrientation == UIDeviceOrientationLandscapeLeft)
        {
            ...
            rect = CGRectMake(rect.origin.y, MSS_RC_SCREEN_WIDTH - rect.size.width - rect.origin.x, rect.size.height, rect.size.width);
        }
        else if(_currentOrientation == UIDeviceOrientationLandscapeRight)
        {
            transform = CGAffineTransformMakeRotation(M_PI / 2);
            rect = CGRectMake(MSS_RC_SCREEN_HEIGHT - rect.size.height - rect.origin.y, rect.origin.x, rect.size.height, rect.size.width);
        }
...
- (void)deviceOrientationDidChange:(NSNotification *)notification
{
    ...
    if(orientation == UIDeviceOrientationPortrait || orientation == UIDeviceOrientationLandscapeLeft || orientation == UIDeviceOrientationLandscapeRight)
    {
        ...
        {
            _screenWidth = MSS_RC_SCREEN_HEIGHT;
            _screenHeight = MSS_RC_SCREEN_WIDTH;
            if(_currentOrientation == UIDeviceOrientationLandscapeLeft)
            {
                [UIView animateWithDuration:0.5 animations:^{
        ...
        _bgView.frame = CGRectMake(0, 0, MSS_RC_SCREEN_WIDTH, MSS_RC_SCREEN_HEIGHT);
